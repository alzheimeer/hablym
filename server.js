...
function serverHandler(request, response) {
    ...
    try {
        ...
        var matched = false;
        ['/www/', '/dev/', '/dist/', '/socket.io/', '/node_modules/canvas-designer/', '/admin/'].forEach(function(item) {
        ...
        ['adapter.js'].forEach(function(item) {
        ...
        try {
            ...
            if (filename.search(/www/g) === -1 && filename.search(/admin/g) === -1 && stats.isDirectory() && config.homePage === '/www/index.html') {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                if (filename.indexOf(resolveURL('/www/MultiRTC/')) !== -1) {
                    filename = filename.replace(resolveURL('/www/MultiRTC/'), '');
                    filename += resolveURL('/www/MultiRTC/index.html');
                ...
                } else if (filename.indexOf(resolveURL('/www/dashboard/')) !== -1) {
                    filename = filename.replace(resolveURL('/www/dashboard/'), '');
                    filename += resolveURL('/www/dashboard/index.html');
                } else if (filename.indexOf(resolveURL('/www/video-conference/')) !== -1) {
                    filename = filename.replace(resolveURL('/www/video-conference/'), '');
                    filename += resolveURL('/www/video-conference/index.html');
                } else if (filename.indexOf(resolveURL('/www')) !== -1) {
                    filename = filename.replace(resolveURL('/www/'), '');
                    filename = filename.replace(resolveURL('/www'), '');
                    filename += resolveURL('/www/index.html');
                } else {
