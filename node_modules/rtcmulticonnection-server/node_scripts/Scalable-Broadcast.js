    ...
    socket.on('join-broadcast', function(user) {
        try {
            ...
            if (relayUser && user.userid !== user.broadcastId) {
                ...
                socket.emit('logs', 'User <' + user.userid + '> is watching the transmission of <' + relayUser.userid + '> User transmission: <' + user.broadcastId + '>');

                // logs for target relaying user
                relayUser.socket.emit('logs', 'You <' + relayUser.userid + '>' + ' forwarding data to <' + user.userid + '> User transmission: <' + user.broadcastId + '>');
                ...
                socket.emit('logs', 'User <' + user.userid + '> is transmitting with hably.');
            }
        } catch (e) {
